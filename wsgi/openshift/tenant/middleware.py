...
def get_user(request):
    if not hasattr(request, '_cached_user'):
        if TENANT_KEY in request.session:
            with tenant_context(get_tenant_model().objects.get(pk=request.session[TENANT_KEY])):
                request._cached_user = auth.get_user(request)
        elif db.connection.schema_name == get_public_schema_name():
            with tenant_context(get_tenant_model().objects.get(schema_name=get_public_schema_name())):
                request._cached_user = auth.get_user(request)
        else:
            request._cached_user = auth.get_user(request)
...
class RestrictTenantStaffToAdminMiddleware(object):
    def process_request(self, request):
        ...
        if reverse('admin:index') in request.path:
            if TENANT_KEY in request.session:
                if request.user.is_staff and not request.session[TENANT_KEY] == db.connection.tenant.id:
